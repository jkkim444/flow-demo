 ...
 */
package com.encocns.fw.config.db;
...
public class SqlMapClientImpl implements SqlMapClient {
    ...
    @Autowired
    @Qualifier("sqlSessionTemplateIFRS")
...
//    protected DataSourceTransactionManager txManagerCore;
     ...
     */
    @Override
    public int insertBatch(String id, Object param) {
        int result = 0;

        try {
            result = sqlSessionTemplateBatch.insert(id, param);
        } catch (Exception e) {
            if(LOGGER.isErrorEnabled()) {
                LOGGER.error(e.getMessage());
            }
     ...
     */
    @Override
    public int updateBatch(String id, Object param) {
        int result = 0;

        try {
            result = sqlSessionTemplateBatch.update(id, param);
        } catch (Exception e) {
            if(LOGGER.isErrorEnabled()) {
                LOGGER.error(e.getMessage());
            }
     ...
     */
    @Override
    public int deleteBatch(String id, Object param) {
        int result = 0;

        try {
            result = sqlSessionTemplateBatch.delete(id, param);
        } catch (Exception e) {
            if(LOGGER.isErrorEnabled()) {
                LOGGER.error(e.getMessage());
            }
    ...
    /**
     * @brief flushStatements
     * @details flushStatements(only batch use)
     * @author : CHOI, YOUNG-HWAN
     * @date : 2016. 10. 25.
     * @see com.encocns.fw.config.db.SqlMapClient#flushStatements()
     * @param void
     * @return <E> void
     */
