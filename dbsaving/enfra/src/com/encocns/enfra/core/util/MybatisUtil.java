 ...
 * @brief Mybatis 관련 Utility이다.
 */
/**
 * @namespace com.encocns.enfra.batch.core.util
 * @brief Utils Package
 */
...
/**
 * @brief Mybatis 관련 Utility
 * @details Mybatis 관련 Utility
 * @author : YH.CHOI
 * @date : 2015. 5. 29. 오후 3:19:32
 * @version : 1.0.0
 */
public class MybatisUtil {
    /**
     * @brief Mybatis의 해당 쿼리를 리턴
     * @details 해당 sqlSession의 MappedStatement에 BoundSql를 이용하여 해당 쿼리에 값을 bind하여 리턴 한다.
     *
     * 사용법 :
     * MappedStatement ms = sqlSessionTemplate.getConfiguration().getMappedStatement("stmt");
     * BoundSql boundSql = ms.getBoundSql(Map params);
     * String finalSql = MybatisUtils.getMybatisQuery(boundSql, params);
     *
     * @author : YH.CHOI
     * @date : 2015. 5. 29.
     * @param paramBoundSql
	...
	public static String getMybatisQuery(BoundSql paramBoundSql, Object params) {

        BoundSql boundSql = paramBoundSql;
            ...
            if(param instanceof Integer || param instanceof Long || param instanceof Float || param instanceof Double){
                ...
                for(ParameterMapping mapping : paramMapping){
                    ...
                    if(value instanceof String){ // SQL의 ? 대신에 실제 값을 넣는다. 이때 String 일 경우는 '를 붙여야 하기땜에 별도 처리
                        ...
                        else sql = sql.replaceFirst("\\?", "'" + value + "'");
                    }else{
                        if(value == null) sql = sql.replaceFirst("\\?", "NULL");
